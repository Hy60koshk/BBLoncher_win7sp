...
#pragma warning disable 649
		...
		public void InitForVersion(string curVer) {
			...
			AvailableMods = Mods.FindAll(mi => mi.LatestVersion != null);
			LauncherConfig.SaveMods();

			/*foreach (ModInfo mi in AvailableMods) {
				foreach (ModVersion mv in mi.Versions) {
					if (mv.Conflicts is null) {
						if (mi.Conflicts != null) {
							foreach (ModDep conf in mi.Conflicts) {
								if (conf.VerId is null) {
									ModInfo cmi = Mods.Find(m => m.Id.Equals(confId));
								}
					...
					if (mi.Dependencies != null && mi.Dependencies.Count > 0) {
						foreach (ModDep[] depIds in mi.Dependencies) {
							if (depIds != null && depIds.Length > 0) {
								List<ModInfo> depSeg = new List<ModInfo>();
								foreach (ModDep depId in depIds) {
		...
		public void NormalizeHashes() {
			if (HasHashes) {
				for (int i = 0; i < Hashes.Count; i++) {
					Hashes[i] = Hashes[i].ToUpperInvariant();
				}
			}
		}
		public bool HasHash(string hash) {
			if (HasHashes) {
				return null != Hashes.Find(x => x.Equals(hash, StringComparison.OrdinalIgnoreCase));
			}
			return false;
		}
	...
	public class ModDep {
		...
		public ModDep(string modId, string verId) {
			ModId = modId;
			VerId = verId;
		}
	}
		...
		internal static List<ModDep[]> parseDependancies(List<string[]> strDeps) {
			if (strDeps != null) {
				...
				foreach (string[] dep in strDeps) {
					List<ModDep> synonims = new List<ModDep>();
					foreach (string mod in dep) {
						string m = mod.Trim();
						if (m.Trim().Length > 0) {
							...
							m = parts[0].Trim();
							if (m.Length > 0) {
								synonims.Add(new ModDep(m, v.Length > 0 ? v : null));
							}
						}
					}
					if (synonims.Count > 0) {
						deps.Add(synonims.ToArray());
					}
				}
				if (deps.Count > 0) {
					return deps;
			...
			if (strCons != null) {
				...
				foreach (string mod in strCons) {
					...
					if (m.Trim().Length > 0) {
						...
						m = parts[0].Trim();
						if (m.Length > 0) {
							cons.Add(new ModDep(m, v.Length > 0 ? v : null));
						}
					}
				}
				if (cons.Count > 0) {
	...
	public class ModInfo {
		...
		public List<ModDep[]> Dependencies = null;
		public List<ModDep> Conflicts = null;
		...
		public List<FileInfo> FilesForUpdate {
			get => MarkedForUpdate ? LatestVersion.Files : CurrentVersion.Files;
		}
		...
		public bool DoesDepend(ModInfo mi) {
			List<ModDep[]> dependencies = CurrentVersion.GetDependencies();
			if (dependencies != null) {
				foreach (ModDep[] deps in dependencies) {
					foreach (ModDep dep in deps) {
						if (mi.DoesSatisfy(dep)) {
							return true;
						}
					}
				}
		...
		public bool DoesConflict(ModInfo mi) {
			List<ModDep> conflicts = CurrentVersion.GetConflicts();
			return conflicts != null && null != conflicts.Find(x => mi.DoesSatisfy(x));
		}

		public bool DoesSatisfy(ModDep modDep) {
			return DoesSatisfy(modDep, false, false);
		}
		public bool DoesLatestSatisfy(ModDep modDep) {
			return DoesSatisfy(modDep, true, false);
		}
		public bool DoesSatisfy(ModDep modDep, bool matchLatest, bool strict) {
			if (Id.Equals(modDep.ModId, StringComparison.OrdinalIgnoreCase)) {
				ModVersion version = matchLatest ? LatestVersion : CurrentVersion;
				if (modDep.VerId is null) {
					return true;
				}
				else if (version.VersionName.Equals(modDep.VerId, StringComparison.OrdinalIgnoreCase)) {
					return true;
				}
				else if (!strict && int.TryParse(modDep.VerId, out int verOrder)) {
					return version.VersionOrder == verOrder;
				}
			}
			return false;
		}
		public ModVersion FindVersionForDep(ModDep modDep) {
			ModVersion cmv = Versions.Find(
				v => v.VersionName.Equals(modDep.VerId, System.StringComparison.OrdinalIgnoreCase)
			);
			if (cmv is null && int.TryParse(modDep.VerId, out int verOrder)) {
				cmv = Versions.Find(
					v => v.VersionOrder == verOrder
				);
			}
			return cmv;
		}

		public bool IsLatestVersionSufficient() {
			List<ModInfo> activeMods = Program.LoncherSettings.AvailableMods.Where(m => m.IsActive).ToList();
			foreach (ModInfo mod in activeMods) {
				List<ModDep> conflicts = LatestVersion.GetConflicts();
				if (conflicts != null) {
					foreach (ModDep conflict in conflicts) {
						if (mod.DoesSatisfy(conflict)) {
							return false;
						}
					}
				}
				conflicts = mod.CurrentVersion.GetConflicts();
				if (conflicts != null) {
					foreach (ModDep conflict in conflicts) {
						if (DoesLatestSatisfy(conflict)) {
							return false;
						}
					}
				}
			}
			List<ModDep[]> dependencies = LatestVersion.GetDependencies();
			if (dependencies != null) {
				foreach (ModDep[] deps in dependencies) {
					bool fulfilled = false;
					foreach (ModDep dep in deps) {
						ModInfo depMod = activeMods.Find(m => m.Id.Equals(dep.ModId, StringComparison.OrdinalIgnoreCase));
						if (depMod != null && (dep.VerId is null || depMod.DoesSatisfy(dep))) {
							fulfilled = true;
							break;
						}
					}
		...
		public List<FileInfo> GetFilesForVersion(string versionName) {
			return Versions.Find(x => x.VersionName == versionName)?.Files;
		}
		public void InitInstallForVersion(string versionName) {
			InitInstallForVersion(Versions.Find(x => x.VersionName == versionName));
		}
		public void InitInstallForVersion(ModVersion version) {
			if (!version.HasVersion()) {
				return;
			}
			CurrentVersion = version;
			SetNamesForVersion(version);
		}
		public void SetNamesForVersion(ModVersion version) {
			VersionedName = Name + ' ' + version.VersionName;
			VersionedDescription = version.Description ?? Description;
			VersionedDetailedDescription = version.DetailedDescription ?? DetailedDescription;
		}

		public void InitForVersion(MainGameVersion gv) {
			...
			if (ModConfigurationInfo is null) {
				List<ModMigration> migratedVariants = Program.LoncherSettings.ModMigrations.FindAll(x => x.NewId.Equals(Id));
				for (int i = 0; i < migratedVariants.Count && ModConfigurationInfo is null; i++) {
					string oldId = migratedVariants[i].OldId;
			...
			CurrentVersion = null;
			LatestVersion = null;
			string exeVersion = Program.GameVersion.ToUpperInvariant();
			AvailableVersions = Versions.FindAll(x => x.HasVersion());
			...
			if (ModConfigurationInfo is null) {
				foreach (ModVersion mv in Versions) {
					FileInfo file = mv.Files[0];
					string fullPath = Program.GamePath + file.Path;
					if (File.Exists(fullPath)) {
						string exeVer = mv.HasVersion() ? exeVersion : mv.ExeVersions[0];
						List<string> fileList = mv.Files.Select(x => x.Path).ToList();
						ModConfigurationInfo = new ModCfgInfo(Id, Name, exeVer, mv.VersionName, fileList, true);
			...
			if (ModConfigurationInfo != null) {
				...
				else if (ModConfigurationInfo.GameVersion.Equals(exeVersion)) {
					CurrentVersion = LatestVersion;
				}
				if (CurrentVersion is null && ModConfigurationInfo.Active && AvailableVersions.Count > 0) {
					List<string> unmatchedFiles = ModConfigurationInfo.FileList.Where(
						file => null == LatestVersion.Files.Find(f => f.Path.Equals(file))
					).ToList();
					bool doFilesMatch = unmatchedFiles.Count < 1;
					bool doFixMod = doFilesMatch || DialogResult.Yes == YobaDialog.ShowDialog(
						String.Format(
							Locale.Get("UnknownModVersionHasDifferentFiles")
							, ModConfigurationInfo.Name, ModConfigurationInfo.ModVersion
						)
						, new UIElement[] {
						new UIElement() {
							Caption = Locale.Get("UpdateBtn"), Result = DialogResult.Yes
						},
						new UIElement() {
							Caption = Locale.Get("DisableMod"), Result = DialogResult.No
						}
						}
					);
					if (doFixMod) {
						ModConfigurationInfo.GameVersion = exeVersion;
						ModConfigurationInfo.FileList = LatestVersion.Files.Select(x => x.Path).ToList();
						CurrentVersion = LatestVersion;

						if (!doFilesMatch) {
							List<FileInfo> oldVersionFiles = new List<FileInfo>();
							foreach (string file in unmatchedFiles) {
								bool fileIsInUse = gv.AllModFiles.Find(f => f.Path.Equals(file)) != null;
								if (!fileIsInUse) {
			...
			if (CurrentVersion == null) {
				...
				else if (AvailableVersions.Count > 1) {
					VersionedName = Name;
					VersionedDescription = Description ?? LatestVersion.Description;
					VersionedDetailedDescription = DetailedDescription ?? LatestVersion.DetailedDescription;
				}
				else {
					SetNamesForVersion(LatestVersion);
				}
			}
			else {
				SetNamesForVersion(CurrentVersion);
				List<FileInfo> files = CurrentVersion.Files;
			...
			if (ModConfigurationInfo != null) {
				if (CurrentVersion is null) {
					...
					foreach (string file in ModConfigurationInfo.FileList) {
						if (CurrentVersion.Files.FindIndex(x => x.Path.Equals(file)) < 0) {
		...
		public void Install() {
			List<string> fileList = CurrentVersion.Files.Select(f => f.Path).ToList();
			ModConfigurationInfo = new ModCfgInfo(Id, Name, Program.GameVersion, CurrentVersion.VersionName, fileList, true);
		...
		public async Task<CheckResult> Enable() {
			...
			try {
				...
				modFileCheckResult = await FileChecker.CheckFiles(CurrentVersion.Files);
		...
		public void DisableOld(MainGameVersion gv) {
			List<FileInfo> oldVersionFiles = new List<FileInfo>();
